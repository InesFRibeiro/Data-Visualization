...
df['win_total'] = np.where(df['position']==1, 1, 0)
df_3 = df.groupby(['circuit', 'constructor' ], as_index=False)['win_total'].sum()
df_3.rename(columns={'circuit':'Circuit'}, inplace=True)

##### Visualization ####################################################################################################

fig_bar = px.histogram(df_3, x="constructor", y="win_total", #color="continent",
                 animation_frame="Circuit", #animation_group="win", 
                 range_y=[0,16],
...
app.layout = html.Div([
    html.H1('Number of wins of each constructor from 1950-2021'),
